...
class HashTable:
    ...
    def check_rehash_and_set_bucket_size(self):
        ...
        if self.item_count > self.bucket_size * 0.7:
            ...
            self.bucket_size = primes[-1]
            return True
        elif (self.item_count < self.bucket_size * 0.3) and (self.item_count >= 97):
            primes = list(primerange(0,self.bucket_size//2 -1))
            if len(primes) == 0:
    ...
    def rehash(self):
        if self.check_rehash_and_set_bucket_size():
            ...
            assert self.item_count == old_item_count  
